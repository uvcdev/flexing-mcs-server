...
const dao = {
  ...
  bulkInsert(paramList: Array<TrackingLogUpsertParams>): Promise<BulkInsertedOrUpdatedResult> {
    return new Promise((resolve, reject) => {
      TrackingLog.bulkCreate(paramList, {
        updateOnDuplicate: ['logId', 'code', 'callId', 'callType', 'subject', 'state', 'startFacility', 'destFacility', 'assignedRobot', 'type', 'value', 'description'],
      })
  ...
  selectList(params: TrackingLogSelectListParams): Promise<SelectedListResult<TrackingLogAttributes>> {
    ...
    if (params.callType) {
      setQuery.where = {
        ...setQuery.where,
        callType: params.callType, // '=' 검색
      };
    }
    if (params.startFacility) {
      setQuery.where = {
        ...setQuery.where,
        startFacility: params.startFacility, // '=' 검색
      };
    }
    if (params.destFacility) {
      setQuery.where = {
        ...setQuery.where,
        destFacility: params.destFacility, // '=' 검색
      };
    }
