...
const tagNames: { name: string; value: string; checked: boolean }[] = Array.from(
  new Set(jsonData[process.env.SITE || 'MBS'].map((tag: any) => tag.TAG_NAME)) // Set에서 중복 제거
).map((tagName) => {
  const tagNameStr = String(tagName);
  const isChecked = jsonData[process.env.SITE || 'MBS'].some(
    (tag: any) => tag.TAG_NAME === tagNameStr && tag.SUBSCRIPTION === true
  );
