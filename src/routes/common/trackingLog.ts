...
router.post(
  '/',
  isLoggedIn,
  async (req: Request<unknown, unknown, TrackingLogInsertParams, unknown>, res: Response) => {
    ...
    try {
      ...
      const params: TrackingLogInsertParams = {
        ...
        leadTimeInfo: {},
        sectionLeadTime: {},
        subjectTimeLog: defaultSubjectTimeLog,
      };
...
router.get(
  '/',
  isLoggedIn,
  async (req: Request<unknown, unknown, unknown, TrackingLogSelectListParams>, res: Response) => {
    const logFormat = makeLogFormat(req);
    const tokenUser = (req as { decoded?: Payload }).decoded;
