...
export const setReceivedAckCommand = (
  systemTopic: string,
  systemName: string,
  callId: string,
  mqttMessage: MbsMqttMesaage
...
const systemAckSubtopicList = ['WMS_ONLINE'];

// 중간 로직을 거치고 나서 보내야 하는 subtopic list
const logicAckSubtopicList = [
  'CALL_REQUEST',
  'ACK_CALL_INFO',
  'ACK_CANCEL_CALL_INFO',
  'ACK_REQ_CALL_INFO_LIST',
  'ACK_REQ_PORT_STATE_LIST',
  'ACK_BRANCH_INFO_REQ',
];

// ACK 호출을 위한 목록 ( WMS에서 받은 데이터 들에 대한 ACK 호출 )
export const checkReceivedAckCommand = async () => {
  const receivedAckCommandList =
    (await redisUtil.hgetAllObject<ReceivedAckCommand>(RedisKeys.ReceivedAckCommandBySubjectCmdId)) || [];
